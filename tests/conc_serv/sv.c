...
typedef struct
{
  char command_udp_h[BYTES_COMMAND_MAX];
  struct sockaddr_in udp_address;
} udp_help;

void *udp_communication_c(void *arg)
{
  char command[BYTES_COMMAND_MAX];
  strcpy(command, ((udp_help *)arg)->command_udp_h);

  struct sockaddr_in skaddr_client = ((udp_help *)arg)->udp_address;
...
void *udp_communication(void *)
{
  ...
  for (; running_condition();)
  {
    ...
    pthread_t th;
    udp_help arg;
    strcpy(arg.command_udp_h, command);
    arg.udp_address = skaddr_client;
    pthread_create(&th, NULL, &udp_communication_c, &arg);
    /*char outcome[BYTES_OUTCOME_MAX];
    strcpy(outcome, "UDP answer to: ");
    strcat(outcome, command);
...
void *tcp_communication_c(void *arg)
{
  int sd = *(int *)arg;
  if (!FD_ISSET(sd, &descriptors.container))
    return NULL;
...
void *multiplexing(void *)
{
  for (; running_condition();)
  {
    ...
    for (int sd = 4;
      ...
      {
        pthread_create(&th[sd], NULL, &tcp_communication_c, &sd);
