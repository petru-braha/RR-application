...
int main()
{
    ...
    char final_number[BYTES_OUTCOME_MAX];
    sprintf(final_number, "%ld", time);
    strcat(final_number, " ");

    // send to a file
    int fd = open("tcp_results.txt", O_APPEND | O_RDWR);
    call(write(fd, final_number, strlen(final_number)));
    call(close(fd));

    printf("cl1 finished with: %s in %ld.\n", outcome, time);    
